...
TEST(TupleOps, FlatGetFromSingleTuple) {
  ...
  EXPECT_EQ((SpanGet<0>(t)), 10);
  EXPECT_DOUBLE_EQ((SpanGet<1>(t)), 3.14);
}
...
TEST(TupleOps, FlatGetFromMultipleTuples) {
  ...
  EXPECT_EQ((SpanGet<0>(t1, t2)), 1);
  EXPECT_DOUBLE_EQ((SpanGet<1>(t1, t2)), 2.0);
  EXPECT_EQ((SpanGet<2>(t1, t2)), "hello");
  EXPECT_EQ((SpanGet<3>(t1, t2)), 'x');
}
